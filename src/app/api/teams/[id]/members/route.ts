...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const requester = await db.teamMember.findFirst({ where: { teamId: params.id, userId: session.user.id } });
  ...
  await db.teamMember.deleteMany({ where: { teamId: params.id, userId } });
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const requester = await db.teamMember.findFirst({ where: { teamId: params.id, userId: session.user.id, role: "ADMIN" } });
  if (!requester) return NextResponse.json({ error: "Only admins can change roles" }, { status: 403 });

  await db.teamMember.updateMany({ where: { teamId: params.id, userId }, data: { role } });
  return NextResponse.json({ ok: true });
}
